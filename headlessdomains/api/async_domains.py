...
class AsyncDomainsAPI(BaseAPI):
    ...
    async def register(self, domain: str, namespace: str, years: int = 1, payment_method: str = "gems", workflows: dict = None) -> Dict[str, Any]:
        """
    ...
    async def update_bio(self, domain: str, workflows: dict = None, **kwargs) -> Dict[str, Any]:
        """
        Update an agent's bio/profile and workflows asynchronously.
        ...
        response = await self.client.post(f"/api/v1/domains/{domain}/bio", json=payload)
        data = self._handle_response(response)
        return data
