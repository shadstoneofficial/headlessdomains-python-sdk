...
class AsyncAgentsAPI(BaseAPI):
    """Asynchronous operations related to Agents."""
    
    async def provision(self, name: str) -> AgentProvisionResult:
        """
        Anonymously provision a new agent without a GFAVIP account.
        Returns credentials including the api_key and claim_code.
        """
        response = await self.client.post("/api/v1/agents/provision", json={"name": name})
